...
void listaDeDivisores (int x, int X[])
{
    for(int i = 1; i <= x; i++)
	{
        if(x % i == 0)
...
int qtdeDeElementosIguais(int x[], int x1, int y[], int y1);
{
	qtdeDeDivisores= x[] + x1 / 2;
	qtdeDeDivisores= y[] + y1/ 2;
	
}

void listaDeDivisoresIguais(int a[], int, int[], int, int[]);
{
	qtdeDeDivisores()
}
