...
ajax.addEventListener("readystatechange", function(){
    if(ajax.readyState === 4 && ajax.status === 200){
        ...
        <div class="container">
                  <div class="img">
                      <img src="${resposta[produtoprice].photo}" alt="">
                  </div>
                  <div class="content">
                      <div class="product-name">${resposta[produtoprice].name}</div>
                      <div class="product-description">${resposta[produtoprice].details}</div>
                      <div class="price">${parseFloat(resposta[produtoprice].price).toLocaleString('pt-BR', {style: 'currency', currency: 'BRL'})}</div>
                      <div class="qty">
        ...
        function addItem() {
            ...
            const id = `${resposta[code].id}`;
            const name = `${resposta[name].titulo}`;
            const qtd = document.getElementById("qtd").value;
            const preco = `${resposta[price].preco}`;
